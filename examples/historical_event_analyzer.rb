...
class HistoricalEventAnalyzer < Lluminary::Task
  ...
  output_schema do
    datetime :event_datetime, description: "The exact date and time when the event occurred in the UTC timezone. If it is known, use the exact time to the minute, otherwise use midnight. Do not guess."
    boolean :exact_time_is_known, description: "Whether the exact time to the minute of the event is known and well documented. If the exact time is not known, return false."
...
if __FILE__ == $0
  ...
  result = HistoricalEventAnalyzer.call(
