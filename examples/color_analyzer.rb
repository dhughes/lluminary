...
class ColorAnalyzer < Lluminary::Task
  ...
  CSS_LEVEL1_COLOR_NAMES = %w[aqua black blue fuchsia gray green lime maroon navy olive purple red silver teal white yellow].freeze

  input_schema do
    string :image_description, description: "A description of an image or scene to analyze for its dominant CSS level 1 color"
    
    validates :image_description, presence: true
  end

  output_schema do
    string :color_name, description: "The lower case CSS level 1 color name that best matches the described image. Originally CSS Level 1 only supported 16 colors, but modern CSS supports many more. We only want one of the 16 original ones."

    validates :color_name, presence: true
    validates :color_name, inclusion: { in: CSS_LEVEL1_COLOR_NAMES, message: "must be a valid CSS level 1 color name" }
    validates :color_name, format: { with: /\A[a-z]+\z/, message: "must be in lowercase" }
  end
